...
class SoapClient:
    ...
    @staticmethod
    def is_wemo_spec(spec: object) -> bool:
        """Return True for device specs that require SOAP transport.

        Checks whether the device was discovered with SOAP control URLs
        from a Wemo/UPnP setup.xml file.
        """
        if not hasattr(spec, "device"):
            return False
        device = spec.device
        return getattr(device, "transport", "") == "upnp"
